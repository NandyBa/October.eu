...
Jusqu'à présent vous avez prêté votre argent à `r count(data) ` projets de `r count(data%>%select(Company.name) %>% distinct()) ` entreprises, pour un montant total de `r sum(data$Investment)`€ et un montant moyen de `r mean(data$Investment)`€ par prêts.
...
Vous avez prêté à des entreprises européennes de `r count(data%>%select(Country) %>% distinct()) ` pays différents. Le tout pour un montant moyen par pays de


`r mean(data_investment_country$total)` euros.
